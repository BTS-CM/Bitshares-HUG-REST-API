...
@hug.get(examples='account=blahblahblah&api_key=API_KEY')
def account_details(account: hug.types.text, api_key: hug.types.text, hug_timer=5):
	"""
	Bitshares account details
	URL: http://HOST:PORT/account_details?account=example_usera&api_key=API_KEY
	"""
	if (check_api_token(api_key) == True): # Check the api key
	# API KEY VALID
